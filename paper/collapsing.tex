...
\begin{align*}
CA(HG({\cal S}), D_1 \sqcup D_1) =  CA(HG({\cal S}), D_2 \sqcup D_2) \; .
\end{align*}
...
\end{scs}
Stated less formally, the conjecture is the following: take any Eulerian solution, double every edge of it, and normalize the resulting solution; then the resulting solution is the same for all possible initial solutions.

Figures~\ref{fig:coll} and~\ref{fig:collnaive}{\todo{AK: Sasha G., these pictures are informative, but take too much space. Should we move them to Appendix?}} illustrate the action of
the Collapsing Algorithm for optimal and naive solutions, respectively. Note when processing level $l>1$ nodes, 
...
We have verified the conjecture on hundreds of thousands datasets (both handcrafted and randomly generated), and we invite the reader to see its visualizations and to check the conjecture on arbitrary datasets at the webpage~\cite{webpage}.
