...
We are now ready to state a~simple and astonishing structural property of the hierarchical graph: take any Eulerian solution, double every edge of it, and normalize the resulting solution; then the resulting solution is the same for all possible initial solutions. Formally, the conjecture is stated as follows.
...
the Collapsing Algorithm for optimal and naive solutions, respectively. When processing level $l>1$ nodes, 
...
We have verified the conjecture on hundreds of thousands datasets (both handcrafted and randomly generated), and we invite the reader to see its visualizations and to check the conjecture on arbitrary datasets at the webpage~\cite{webpage}. Moreover, we prove the conjecture for the (NP-hard) special case when input strings have length at most~3 in Section~\ref{subsec:scs3}.
