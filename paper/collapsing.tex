...
We are now ready to state a~simple and astonishing structural property of the hierarchical graph: take any Eulerian solution, double every edge of it, and normalize the resulting solution; the resulting solution is the same for all possible initial solutions. For the formal statement of the conjecture we use the following notation:  If $U$ and $V$ are two multisets, then $U\sqcup V$ is the multiset $W$ such that each $w\in W$ has multiplicity equal to the sum of multiplicities it has in sets $U$ and $V$.
