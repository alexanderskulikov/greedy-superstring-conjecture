...
A~permutation $(s_{i_1}, \dotsc, s_{i_n})$ gives a~``zig-zag'' Eulerian solution:
\[\varepsilon \to s_{i_1} \to \overlap(s_{i_1}, s_{i_2}) \to s_{i_2} \to
\overlap(s_{i_2}, s_{i_3}) \to \dotsb \to \overlap(s_{i_{n-1}}, s_{i_n}) \to s_{i_n} \, .\]
This Eulerian solution is shown schematically in Figure~\ref{fig:hgperm}(a). This schematic illustration is over simplified as the shown path usually intersect many times. Still, this point of view is helpful in understanding the algorithms presented later in the text. 
...
structure described above: there are many other solutions. In the next section, we introduce a~normalization procedure (that we call collapsing)
...
Imagine that all arcs of~$D$ are one~circular thread, and that there is a~nail in every node~$s \in {\cal S}$ corresponding to an input string. We apply {\em gravitation} to the thread, i.e., we replace every pair of arcs $(\pref(v), v)$, $(v, \suff(v))$ 
