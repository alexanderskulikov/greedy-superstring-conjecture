...
It is interesting to note that any normalized solution is of the form~\ref{eq:zigzag}, but not vice versa: see Fig.~\ref{fig:abnormalzigzag}.
Hence, the normalization procedure not just turns a~solution into some standard form, but may also improve it.
...
\caption{(a)~An~Eulerian solution corresponding for a~permutation $({\tt ae}, {\tt ca})$. (b)~Normalizing it, results in a~shorter solution corresponding to a~permutation~$({\tt ca}, {\tt ae})$.}
\end{figure}
