...
than just all pairwise overlaps. Given an instance of SCS, the vertex set of the corresponding hierarchical graph is just the set of substrings of all the given strings. For a string $s$ and two symbols $\alpha, \beta$, the graph contains the arcs: $(s,s\alpha)$ and $(\beta s, s)$. Now, every superstring of the given set of string corresponds to an Eulerian walk in the hierarchical graph. We define the following simple normalization procedure of a walk in the hierarchical graph: replace the pair of arcs $(\alpha s, \alpha s \beta), (\alpha s \beta, s\beta)$ with the pair $(\alpha s, s ), (s , s\beta)$ as long as it does not violate connectivity of the walk. It is easy to see that normalization never increases the length of the corresponding solution of SCS. 

We observe a surprising property of this normalization procedure: if one takes \emph{any} solution, doubles every its arc in the hierarchical graph, and then apply the normalization procedure, then the resulting set of arcs is always the same (i.e., it does not depend on the initial solution).
