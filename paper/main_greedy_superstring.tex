...
Gallant et al.~\cite{GMS1980} show that SCS on strings of length $3$ is $\mathbf{NP}$-hardt, while SCS on strings of length at most $2$ is solvable in polynomial time. In this section we show that GHA finds an optimal solution in this case as well.

\begin{lemma}
Let $S=\{s_1,\ldots,s_n\}$ be a set of strings of length $n$, and let $s$ be an optimal superstring for $S$. Then $GHA({\cal S})$ returns a~superstring of length at most~$|s|$. 
\end{lemma}
