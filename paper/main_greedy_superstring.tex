...
\tikzstyle{arraynodes}=[draw=gray,minimum size=5mm,inner sep=0mm,anchor=center]
\tikzstyle{h}=[arraynodes,fill=mybluefill]
\tikzstyle{myarray}=[draw=none,matrix of nodes,nodes={arraynodes},ampersand replacement=\&,line width=.3mm,row sep=-\pgflinewidth,column sep=-\pgflinewidth]
...
$2\frac{11}{23}$ due to Mucha~\cite{M2013} and $2\frac{11}{30}$ due to Paluch~\cite{P14}. See \cite[Section~2.1Z]{GKM13} for an overview of the 
previously known approximation algorithms
and inapproximability results. 
The corresponding algorithms and their analyzes are complicated.
At the same time, an~intriguing {\em greedy conjecture} remains open
for about 30 years already. It states that the following simple 
...
%\end{definition}
%
Figure~\ref{fig:hgexfirst} gives an example of the hierarchical graph as well as shows that the terminology of up- and down-edges comes from placing all the strings of the same length at the same layer where the $i$-th layer contains strings of length~$i$. 
...
\end{scope}
\end{mypic}
\caption{Left: The hierarchical graph for  
$\mathcal{S}=\{{\tt abba}, {\tt abca}, {\tt dbb}, {\tt bab}, {\tt cab}, {\tt aac}\}$. The strings from $\mathcal{S}$ are given in rectangles. Right: TBW}\label{fig:hgexfirst}
\end{figure}
...
This is just because each arc going up has 
...
\end{scope}
\end{mypic}
\caption{Collapsing a~pair of edges is replacing a~pair of dashed edges with a~pair of solid edges: general case (left) and example (right). A~``physical'' meaning of this transformation is that to get {\tt bac} from {\tt aba} one needs to cut~{\tt a} from the beginning and then append~{\tt c} to the end and these two operations commute.}
